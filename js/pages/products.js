$(document).ready(function () {   
    ...
    function bindProducts(productList){
        for(let i=0;i<productList.length;i++){
            $('#productTable').append('<tr><td>'+(i+1)+'</td><td>'+productList[i].Category+'</td><td>'+productList[i].Product+'</td></tr>');
        }
    ...
    function bindProductIntermediate(productList){
        for(let i=0;i<productList.length;i++){
            $('#intermediateTable').append('<tr><td>'+(i+1)+'</td><td>'+productList[i].API+'</td><td>'+productList[i].APICASNo+'</td><td>'+productList[i].NAME+'</td><td>'+productList[i].CASNo+'</td></tr>');
        }
    ...
    $('#txtSearchInput').on('change keydown paste input',(e)=>{
        ...
        if(inputText === ''){
            ...
            let filterProductList=api.filter((data)=>data.Product.toLowerCase().includes(inputText.toLowerCase()))            
            if(filterProductList.length === 0 ) {
                $('#productTable').html('<tr><td colspan="3">No data found!</td></tr>');
            } else {
                bindProducts(filterProductList)
            }
            let filterIntermediate=intermediate.filter((data)=>data.NAME?.toLowerCase().includes(inputText.toLowerCase()))            
            if(filterIntermediate.length === 0 ) {
                $('#intermediateTable').html('<tr><td colspan="5">No data found!</td></tr>');
            } else {
